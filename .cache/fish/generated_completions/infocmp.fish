...
complete -c infocmp -s c -d 'lists each capability that two entries have in R common "'
complete -c infocmp -s n -d 'lists capabilities that are in  none of the given entries'
...
complete -c infocmp -s I -d 'option.  If more than one is specified, the program assumes the'
complete -c infocmp -s d -d option
...
complete -c infocmp -s q -d 'option to distinguish absent and canceled capabilities; see %terminfo(5)'
complete -c infocmp -s u -d 'option is further specified, infocmp rewrites the description of the first ty…'
complete -c infocmp -s x -d 'option to add BSD-compatibility capabilities (names prefixed with “Use= Optio…'
