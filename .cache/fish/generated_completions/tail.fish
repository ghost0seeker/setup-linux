...
complete -c tail -s f -l follow -d 'output appended data as the file grows; an absent option argument means \'desc…'
complete -c tail -s F -d 'same as --follow=\\,name\\/ --retry'
complete -c tail -s n -l lines -d 'output the last NUM lines, instead of the last 10; or use -n +NUM to skip NUM…'
complete -c tail -l max-unchanged-stats -d 'with --follow=\\,name\\/, reopen a FILE which has not changed size after N (def…'
complete -c tail -l pid -d 'with -f, terminate after process ID, PID dies; can be repeated to watch multi…'
